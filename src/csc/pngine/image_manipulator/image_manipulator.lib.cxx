...
image_manipulator::image_manipulator(
  ...
  m_clipping_pipeline = m_device->get().createComputePipelineUnique(vk::PipelineCache{}, clipping_pipeline_info, nullptr).value;
  ...
  m_scaling_pipeline = m_device->get().createComputePipelineUnique(vk::PipelineCache{}, scaling_pipeline_info, nullptr).value;
...
pngine::image_manipulator_bundle image_manipulator::scale_image(float scaleX, float scaleY) {
  ...
  const auto second_image_size = vk::Extent2D(std::floorf(m_first_image_size.width * scaleX), std::floorf(m_first_image_size.height * scaleY));
  ...
  /* bindDescriptorSets */
  compute_buffer.bindDescriptorSets(vk::PipelineBindPoint::eCompute, m_compute_pipeline_layout.get(), 0u, 1u, &scale_descr_set.get(), 0u, nullptr);
  /* dispatch */
