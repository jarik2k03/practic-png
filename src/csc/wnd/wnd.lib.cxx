...
void switch_from_insert_to_normal(wnd::controller*& to_fill, pngine::pngine_core*& render, png::picture*& image_data, bool first_btn, bool work_action, bool abort_action) {
  ...
  if (first_btn && (work_action || abort_action)) { // abort-нажатие только переключит состояние, а work переключит после работы
...
export void character_event(wnd::glfw_window* current, uint32_t codepoint) noexcept {
  ...
  if ((codepoint >= '0' && codepoint <= '9') || codepoint == '-' || codepoint == '.' || codepoint == ' ' || codepoint == '+') // ограничиваем
    controller_obj->input_data.push_back(static_cast<char>(codepoint));
}

export void choosing_tool_by_keyboard_event(wnd::glfw_window* current, int key, int /*scancode*/, int action, int /*mods*/) noexcept {
  const auto& window_obj = *reinterpret_cast<wnd::window_handler*>(&current); // пока так можно делать
  auto& [pngine_obj, picture_obj, controller_obj] = *reinterpret_cast<wnd::pngine_picture_input_package*>(window_obj.get_user_pointer());
  ...
  wnd::switch_from_normal_to_insert(controller_obj, is_key_on_clip_btn, is_key_on_scale_btn, is_key_on_rotate_btn, is_one_of_there_btns_clicked, is_press_action);
}

export void applying_tool_by_keyboard_event(wnd::glfw_window* current, int key, int /*scancode*/, int action, int /*mods*/) noexcept {
  const auto& window_obj = *reinterpret_cast<wnd::window_handler*>(&current); // пока так можно делать
  auto& [pngine_obj, picture_obj, controller_obj] = *reinterpret_cast<wnd::pngine_picture_input_package*>(window_obj.get_user_pointer());

  if (key == GLFW_KEY_BACKSPACE && !controller_obj->input_data.empty() && (action == GLFW_REPEAT || action == GLFW_PRESS))
  ...
  wnd::switch_from_insert_to_normal(controller_obj, pngine_obj, picture_obj, is_key_on_apply_btn, is_accept_action, is_decline_action);
}
...
export void applying_tool_by_mouse_event(wnd::glfw_window* current, int button, int action, int /*mods*/) noexcept {
  ...
  wnd::switch_from_insert_to_normal(controller_obj, pngine_obj, picture_obj, is_mouse_on_apply_btn, is_accept_action, is_decline_action);
}
